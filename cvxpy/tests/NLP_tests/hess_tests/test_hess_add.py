...
class TestHessAdd():
    ...
    def test_three_atoms_different_variables(self):
        ...
        assert(np.allclose(result_dict[(x, x)].toarray(), result_correct_xx))
        assert(np.allclose(result_dict[(y, y)].toarray(), result_correct_yy))
        assert(np.allclose(result_dict[(z, z)].toarray(), result_correct_zz))
